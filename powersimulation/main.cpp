...
int main(int argc, char** argv) {
 ...
 void* SocketHandler(void* lp){
     ...
     getline(iss,line);
     if(line !=""){
     if(line.compare("Create new session:0\r")==0){
         // find out the GUI clien ipaddress.
         struct sockaddr_in addr;
         socklen_t addr_size = sizeof(struct sockaddr_in);
         int res = getpeername(newsockfd, (struct sockaddr *)&addr, &addr_size);
         char *clientip = new char[20];
         strcpy(clientip, inet_ntoa(addr.sin_addr));
         string gui_client_ip(clientip);
         cout<<"GUI client ip address is:"<<endl;
         cout<<gui_client_ip<<endl;
         /*
         ...
         isTestbed=0;
         is_batch=false;
         string data(receive_buffer+sizeof("Create new session:0\r"));
         if(pre!=NULL)
           delete pre;
         PowerSimulator *ps=new PowerSimulator();
         pre=ps;
         (*ps).initialize((char*)data.c_str());
	 (*ps).gui_client_ip=gui_client_ip;
     ...
     else if(line.compare("Create new session:1\r")==0){
         isTestbed=1;
         is_batch=false;
         close(newsockfd);
         string data(receive_buffer+sizeof("Create new session:1\r"));
         if(pre!=NULL)
           delete pre;
         forwarder *ps=new forwarder();
          pre=ps;
         (*ps).initialize((char*)data.c_str());
         (*ps).parse();
         (*ps).forward();
         (*ps).getTestbedStatus(&mutexsum);
         cout<<"Testbed session has been created!"<<endl;     
         bzero(receive_buffer,SOCKET_BUFFER_SIZE);
     }
     ...
     else if(line.compare("Unregistered from the coordinator.\r\n")==0) {
         cout<<"Unregistered from the coordinator"<<endl;
         pthread_mutex_lock (&mutexsum);
         (*pre).restore();   
         pthread_mutex_unlock (&mutexsum);
         bzero(receive_buffer,SOCKET_BUFFER_SIZE);
