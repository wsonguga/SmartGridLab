...
array set widgets {
    ...
    "Current"
	{ widget_current_config widget_current_init widget_current_periodic widget_current_move}
}
...
proc widget_loop { } {
    global oper_mode widget_loop_ID node_list
    set c .c
    ...
    #stan update the switch power interface together with power line.
...
proc getstats_link_ifname { link } {
    ...
    if { [lindex $emulp 1] == "cored" } {
	# TODO: need to determine session number used by cored
...
proc widget_adjacency_periodic { now } {
	...
	# when using cored, send Execute Message and draw line using
