...
proc pluginsCapConfigRefreshList { wi } {
	...
	# skip CORE daemons
	if { [lsearch -exact "openvz cored" $capname] != -1 } { continue }
...
proc sessionConfig { cmd wi channel } {
    global g_current_session power_line_list
...
proc connectShutdownSession { cmd channel sid } {
    ...
    set thumb ""
    set user $CORE_USER
    #stan session number should be used as parameter in the future
...
proc loadPluginsConf { } {
	...
	# update legacy cored.py name - may be removed in the future
	if { $plugin == {"cored.py"} } { set plugin {"cored"} }
