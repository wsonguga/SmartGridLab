...
proc apiNodeCreate { node vals_ref } {
    global $node nodetypes node_list canvas_list curcanvas eid power_node_list
    upvar $vals_ref vals
...
#stan: draw node after the model is set.
    set pnode "p$node"
    set power_interface_flag [getNodePowerInterfacesFlag $pnode]
    set current [getNodeCurrent $pnode]
    set desired_current [getNodeDesiredCurrent $pnode]
    set max_current [getNodeMaxCurrent $pnode]
    set power_interfaces [getNodePowerInterfaces $pnode]
    setNodeCurrent $node $current
    setNodeDesiredCurrent $node $desired_current
    setNodeMaxCurrent $node $max_current
    setNodePowerInterfaces $node $power_interfaces
    setNodePowerInterfacesFlag $node $power_interface_flag
    if { $model == "Storage" } {
      set e_stored [getNodeEStored $pnode]
      set capacity [getNodeCapacity $pnode]
      set operation_mode [getNodeOperationMode $pnode]
...
proc parseLinkMessage { data len flags } {
    ...
    array set typesizes {	node1num 4 node2num 4 delay 8 bw 8 per 8 \
			dup 2 jitter 2 mer 2 burst 2 session -1 \
    ...
    array set vals {	node1num -1 node2num -1 delay 0 bw 0 per 0 \
			dup 0 jitter 0 mer 0 burst 0 session "" \
...
proc exec_shell_callback { node execnum execcmd execres execstatus } {
    ...
    #stan
    ##sh-4.2 because invoked from bash 4.2. add ip list.
	# command: ip addr to see the ip address.
    global isTestbed
    global IPADDRESSES
    if { $isTestbed == 1} {
	exec ssh -Y root@$IPADDRESSES($node) xterm -T "BeagleBoard-$node" &
    #-stan
    } else {
    set title "ScorePlus: [getNodeName $node] (console)"
...
proc sendSessionProperties { sock } {
    global currentFile LIBDIR CORE_USER
...
# ask cored to load or save an XML file based on the current session
...
proc buildStringTLV { type data len_ref } {
	...
	set data_len 65536
	set data [string range $data 0 65535]
    }
    set data_pad_len [pad_32bit $data_len]
    set data_pad [binary format x$data_pad_len]
